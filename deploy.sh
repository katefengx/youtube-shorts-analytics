...
# Copy built files (excluding large media files)
echo "📋 Copying files..."
...
# Copy only small assets (exclude videos)
mkdir -p docs/assets
cp static/assets/vite.svg docs/assets/ 2>/dev/null || true
...
echo ""
echo "⚠️  Note: You'll need to deploy your Flask backend separately and update the API URLs in config.js" 